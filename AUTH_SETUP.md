...
Когда пользователь открывает приложение из Telegram-бота, он автоматически авторизуется:
- `UserContext` проверяет наличие Telegram WebApp данных
- Вызывает API `/api/auth/telegram-user`
- API создает пользователя (если новый) или находит существующего
